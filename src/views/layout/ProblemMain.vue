...
// 计算问题和回答的序号
const getQuestionIndex = (index: number) => Math.ceil((index + 1) / 2)
const getAnswerIndex = (index: number) => Math.floor(index / 2) + 1
</script>
            ...
            <div class="message-list">
                <div v-for="(message, index) in messages" :key="message.id" :class="['message', message.type]">
                    <div class="message-content">
                        <span v-if="message.type === 'user'" class="message-prefix">Q{{ getQuestionIndex(index)
                            }}</span>
                        <span v-if="message.type === 'ai'" class="message-prefix">A{{ getAnswerIndex(index) }}</span>
